    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Oops!</h2>
    ...
    <div className="min-h-screen bg-[#F7F8FA] pb-24">
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="font-bold text-slate-800 dark:text-slate-200 line-clamp-1">{exam.title}</h1>
          </div>
          
          <div className="flex items-center gap-6">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold text-sm transition-colors ${isTimeLow ? 'bg-rose-100 text-rose-600 animate-pulse' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
      ...
      {exam.pdf_url ? (
        <main className="max-w-[1600px] mx-auto w-full h-[calc(100vh-64px)] flex overflow-hidden">
          ...
          <div className="w-1/2 h-full border-r border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 p-4">
            <iframe 
          ...
          <div className="w-1/2 h-full overflow-y-auto p-8 pb-32 space-y-6 custom-scrollbar">
            {questions.map((q, idx) => (
              <div key={q.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm relative">
                    ...
                    {q.type === 'MULTIPLE_CHOICE' && q.options && q.options.length > 0 ? (
                      <div className="space-y-2 mt-4">
                        {q.options.map((opt, oIdx) => {
                          ...
                          return (
                            <label 
                              key={oIdx} 
                              className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-colors ${isSelected ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-200 hover:bg-slate-50 dark:bg-slate-950'}`}
                            >
                      ...
                      <div className="mt-4">
                        <input 
                          ...
                          className="w-full p-4 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-800 dark:text-slate-200 font-medium"
                          value={answers[q.id] || ''}
        ...
        <main className="max-w-4xl mx-auto px-6 mt-8 space-y-6 pb-32">
          {questions.map((q, idx) => (
            <div key={q.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-950 text-indigo-600 font-bold flex items-center justify-center shrink-0">
                  {q.question_number}
                </div>
      ...
      {/* Bottom Action Bar */}
      <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 p-4 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-40">
