    ...
    <div className="min-h-screen bg-[#F7F8FA] pb-24">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        ...
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Button variant="ghost" onClick={() => router.push('/dashboard/grammar')} className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">
            Exit to Dashboard
          </Button>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 mt-12 space-y-8">
        {/* Score Card */}
        <section className="bg-white dark:bg-slate-900 rounded-[2rem] p-10 text-center shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden">
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-emerald-50 dark:bg-emerald-950 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h2 className="text-xl font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Final Score</h2>
            <div className="text-7xl font-black text-slate-800 dark:text-slate-200 tracking-tighter mb-4">
              {result.percentage}%
            </div>
        ...
        {/* Detailed Breakdown */}
        <section>
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-6">Detailed Breakdown</h3>
          <div className="space-y-4">
            {result.questionResults?.map((qr: any, idx: number) => (
              <div key={idx} className={`bg-white dark:bg-slate-900 border rounded-2xl p-6 shadow-sm ${qr.is_correct ? 'border-emerald-200' : 'border-rose-200'}`}>
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {!qr.is_correct && (
                        <div className="bg-emerald-50 dark:bg-emerald-950 p-4 rounded-xl border border-emerald-100">
