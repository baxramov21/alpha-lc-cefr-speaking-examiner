...
export default function ExamSessionPage() {
  ...
  // Part info
  const currentPart = EXAM_PARTS.find((p) => p.part === question.part) || { label: 'Part', description: 'Speaking Exam', color: 'bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 ', questionRange: '' };
  ...
  if (!question || isRestoring) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex items-center justify-center p-6">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex flex-col">
      {/* ---- Top Navigation Bar ---- */}
      <header className="bg-white dark:bg-slate-900 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-sm sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
                ...
                <div
                  className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold transition-all ${partIndex === idx
                    ...
                    : partIndex > idx
                      ...
                      : 'bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 text-slate-400 dark:text-slate-500 dark:text-slate-500 border-2 border-slate-200 dark:border-slate-700 dark:border-slate-700 '
                    }`}
                >
                  {partIndex > idx ? '✓' : idx + 1}
                </div>
                {idx < EXAM_PARTS.length - 1 && (
                  <div className={`w-6 h-0.5 rounded-full ${partIndex > idx ? 'bg-teal-400' : 'bg-slate-200 dark:bg-slate-700 dark:bg-slate-700 '}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-xs text-muted-foreground font-medium bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 px-3 py-1.5 rounded-lg">
        ...
        {/* Progress bar */}
        <div className="h-1 bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 ">
          <div
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        ...
        {isSubmitting && (
          <div className="absolute inset-0 z-50 bg-white dark:bg-slate-900 dark:bg-slate-900 flex flex-col items-center justify-center fade-in px-4">
            {evalError ? (
              <div className="flex flex-col items-center">
                 ...
                 <h2 className="text-3xl font-extrabold text-slate-800 dark:text-slate-200 dark:text-slate-200 tracking-tight text-center">
                    Evaluation Failed
                 </h2>
              ...
              <>
                ...
                <p className="text-lg text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-3 text-center max-w-md leading-relaxed">
                ...
                <div className="mt-10 flex flex-col gap-4 w-full max-w-sm">
                  <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-sm">
                    <CheckCircle2 className="w-6 h-6 text-teal-500 shrink-0" />
                    <span className="font-medium text-slate-700 dark:text-slate-300 dark:text-slate-300 ">Audio processing complete</span>
                  </div>
                  <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-sm opacity-80">
                    <Loader2 className="w-6 h-6 text-amber-500 animate-spin shrink-0" />
                    <span className="font-medium text-slate-700 dark:text-slate-300 dark:text-slate-300 ">Analyzing CEFR criteria...</span>
                  </div>
                </div>
              </>
            )}
        ...
        <div className="w-full max-w-2xl">
          {question.part === 'part3' ? (
            ...
            <div className="flex flex-col gap-6 fade-in w-full max-w-4xl mx-auto">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-6">
                  ...
                  />
                  {phase === 'prep' && (
                    <div className="flex-1 max-w-md bg-white dark:bg-slate-900 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm rounded-full h-12 flex items-center justify-between px-6">
                      <span className="text-slate-500 dark:text-slate-400 dark:text-slate-400 font-medium text-sm">
                        then <strong className="text-slate-800 dark:text-slate-200 dark:text-slate-200 ml-1">Speak - {Math.floor(question.speakSeconds / 60)}:{(question.speakSeconds % 60).toString().padStart(2, '0')}</strong>
                      </span>
                  ...
                  {phase === 'speak' && (
                    <div className="flex-1 max-w-md bg-white dark:bg-slate-900 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm rounded-full h-12 flex items-center px-6 gap-2">
                      <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                      <span className="text-teal-700 font-bold text-sm flex-1">Recording...</span>
                      {allowSkip && <Button onClick={advanceQuestion} variant="outline" size="sm" className="h-8 rounded-full text-xs bg-emerald-50 dark:bg-emerald-950 dark:bg-emerald-950 text-emerald-700 border-emerald-200 hover:bg-emerald-100">Finish Exam</Button>}
                    </div>
                  )}
                </div>
              </div>
              ...
              {question.tableData && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                  {/* FOR Column */}
                  <div className="border-2 border-emerald-500 rounded-xl p-5 bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-sm relative">
                    ...
                    <ul className="mt-2 space-y-3">
                      {question.tableData.forPoints.map((pt, i) => (
                        <li key={i} className="flex items-start gap-3">
                          ...
                          <span className="text-slate-700 dark:text-slate-300 dark:text-slate-300 text-sm leading-relaxed">{pt}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  {/* AGAINST Column */}
                  <div className="border-2 border-red-500 rounded-xl p-5 bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-sm relative">
            ...
            <>
              ...
              {/* Question card */}
              <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-xl shadow-slate-100/80 p-8 mb-6 fade-in flex flex-col gap-6">
                {question.part === 'part2' ? (
                  <div className="flex flex-col md:flex-row gap-6 items-stretch">
                    {question.imageUrl && (
                      <div className="w-full md:w-5/12 relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex items-center justify-center shrink-0">
                        ...
                        {(() => {
                          ...
                          return bullets.map((bullet, idx) => (
                            <li key={idx} className="flex items-start gap-3">
                              ...
                              <span className="text-slate-800 dark:text-slate-200 dark:text-slate-200 text-[17px] leading-relaxed">{bullet}</span>
                            </li>
                          ));
                        })()}
                      </ul>
                  ...
                  <>
                    {question.part === 'part1_2' && currentIndex === examQuestions.findIndex(q => q.part === 'part1_2') ? (
                      <div className="flex flex-col md:flex-row gap-4 w-full">
                        {question.imageUrl && (
                          <div className="relative flex-1 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex items-center justify-center">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
              ...
              <div className="flex items-center justify-between gap-6">
                ...
                <div className="flex-1">
                  {phase === 'prep' ? (
                    <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-sm p-6 flex flex-col items-center justify-center gap-4 h-full min-h-[140px]">
                      <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400 text-center">Take a moment to read the question and prepare your answer.</p>
                      {allowSkip && (
                        <Button
                          onClick={skipPrep}
                          variant="outline"
                          className="border-slate-200 dark:border-slate-700 dark:border-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 hover:bg-slate-50 rounded-xl"
                        >
                    ...
                    <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 dark:border-slate-800 shadow-sm p-5 flex flex-col items-center gap-3">
                      {/* Live waveform */}
            ...
            {examQuestions.map((q, idx) => (
              <div
                ...
                className={`w-1.5 h-1.5 rounded-full transition-colors ${idx === currentIndex
                  ...
                  : idx < currentIndex
                    ? 'bg-emerald-500'
                    : 'bg-slate-200 dark:bg-slate-700 dark:bg-slate-700 '
                  }`}
              />
            ))}
          </div>
