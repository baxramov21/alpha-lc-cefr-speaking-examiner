...
export default function ExamSetupPage() {
  ...
  if (isLoadingQuestions) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex flex-col items-center justify-center p-6 fade-in">
        <div className="flex flex-col items-center justify-center space-y-8 max-w-sm w-full">
          <div className="relative flex items-center justify-center w-28 h-28">
            {/* Outer rings */}
            <div className="absolute inset-0 border-4 border-slate-100 dark:border-slate-800 dark:border-slate-800 rounded-full shadow-inner"></div>
          ...
          <div className="text-center space-y-3 w-full bg-white dark:bg-slate-900 dark:bg-slate-900 p-6 rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 dark:border-slate-800 dark:border-slate-800 relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200 dark:text-slate-200 tracking-tight">Preparing Exam</h2>
            <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 text-sm font-medium">Loading and securing randomized questions...</p>
            <div className="w-full bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 h-2 rounded-full mt-4 overflow-hidden">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950  flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 dark:bg-slate-900  rounded-[var(--radius-lg)] shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800 dark:border-slate-800 ">
            ...
            {(['overview', 'mictest', 'ready'] as SetupStep[]).map((s) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-all ${
                  step === s ? 'bg-white dark:bg-slate-900 dark:bg-slate-900 scale-125' : 'bg-white/40'
                }`}
              />
            ))}
          </div>
        ...
        <div className="p-8">
          ...
          <div className="space-y-8">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-600 dark:text-slate-300 dark:text-slate-300  leading-relaxed mb-3">
                  Questions 1-6. You will have 5-10 seconds to prepare and 30-45 seconds to answer each question.
                </p>
                <div className="flex gap-3 text-xs">
                  <div className="bg-amber-50 dark:bg-amber-950 dark:bg-amber-950 border border-amber-200 px-2.5 py-1 rounded-lg text-amber-700 font-medium">5-10s prep</div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-3 text-xs">
                  <div className="bg-amber-50 dark:bg-amber-950 dark:bg-amber-950 border border-amber-200 px-2.5 py-1 rounded-lg text-amber-700 font-medium">60s prep</div>
            ...
            <div className="flex items-start gap-4">
              <div className="bg-amber-50 dark:bg-amber-950 dark:bg-amber-950 text-amber-600 p-3 rounded-xl mt-1">
            ...
            {!isFullExam && (
              <div className="flex items-start gap-4 mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 dark:border-slate-800">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-slate-900 mb-3">Select Exam Mode</h3>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <button 
                      onClick={() => setExamMode('full')}
                      className={`p-3 rounded-xl border text-sm font-semibold transition-all ${examMode === 'full' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white dark:bg-slate-900 dark:bg-slate-900 border-slate-200 dark:border-slate-700 dark:border-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 hover:border-slate-300'}`}
                    >
                      Full Exam
                    </button>
                    <button 
                      onClick={() => setExamMode('part1')}
                      className={`p-3 rounded-xl border text-sm font-semibold transition-all ${examMode === 'part1' ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white dark:bg-slate-900 dark:bg-slate-900 border-slate-200 dark:border-slate-700 dark:border-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 hover:border-slate-300'}`}
                    >
                      Part 1 Only
                    </button>
                    <button 
                      onClick={() => setExamMode('part2')}
                      className={`p-3 rounded-xl border text-sm font-semibold transition-all ${examMode === 'part2' ? 'bg-violet-600 border-violet-600 text-white' : 'bg-white dark:bg-slate-900 dark:bg-slate-900 border-slate-200 dark:border-slate-700 dark:border-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 hover:border-slate-300'}`}
                    >
                      Part 2 Only
                    </button>
                    <button 
                      onClick={() => setExamMode('part3')}
                      className={`p-3 rounded-xl border text-sm font-semibold transition-all ${examMode === 'part3' ? 'bg-amber-600 border-amber-600 text-white' : 'bg-white dark:bg-slate-900 dark:bg-slate-900 border-slate-200 dark:border-slate-700 dark:border-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 hover:border-slate-300'}`}
                    >
                      Part 3 Only
                    </button>
                  </div>
          ...
          <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800 dark:border-slate-800 flex items-center justify-between">
            <Button 
              variant="ghost" 
              className="text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-900"
            ...
            {step === 'mictest' && (
              <div className="fade-in">
                ...
                <h2 className="text-xl font-black text-slate-800 dark:text-slate-200 dark:text-slate-200  mb-1.5">Test Microphone</h2>
            ...
            {step === 'ready' && (
              <div className="fade-in text-center">
                ...
                <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200 dark:text-slate-200  mb-2">All set!</h2>
                <p className="text-muted-foreground  mb-8">
                  Your microphone is working. The exam will start when you press the button below.
                </p>

                <div className="bg-slate-50 dark:bg-slate-950 dark:bg-slate-950  rounded-2xl p-5 mb-8 text-left space-y-2">
                  ...
                  {[
                    ...
                    <div key={item.label} className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 ">{item.value}</span>
                    </div>
                  ))}
                </div>
