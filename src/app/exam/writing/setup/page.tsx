    ...
    const fetchWritingQuestions = async () => {
      try {
        ...
        const selectedQuestions = [...task1_1Questions, ...task1_2Questions, ...task2Questions].map(q => {
          ...
          let title = 'Task 1.1';
          let minW = q.speak_seconds || 50;
          let recMin = q.prep_seconds || 10;

          if (q.part === 'task1_2') {
            taskNumber = 1.2;
            title = 'Task 1.2';
            minW = q.speak_seconds || 50;
            recMin = q.prep_seconds || 15;
          } else if (q.part === 'task2') {
            taskNumber = 2;
            title = 'Part 2';
            minW = q.speak_seconds || 180;
            recMin = q.prep_seconds || 35;
          }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-[var(--radius-lg)] shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800 dark:border-slate-800">
        ...
        <div className="p-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-300 dark:text-slate-300 leading-relaxed">
                  You have exactly 60 minutes to complete Task 1.1, Task 1.2, and Part 2. You should spend about 10 minutes on Task 1.1, 15 minutes on Task 1.2, and 35 minutes on Part 2.
                </p>
              </div>
            </div>
          ...
          {/* Action Area */}
          <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800 dark:border-slate-800 flex flex-col items-center justify-between gap-4 md:flex-row">
            <Button 
              variant="ghost" 
              className="text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-900 w-full md:w-auto order-2 md:order-1"
            ...
            <div className="w-full md:w-auto order-1 md:order-2 flex flex-col items-center gap-4">
              ...
              {questionsError && (
                <p className="mt-2 text-sm text-red-500 text-center bg-red-50 dark:bg-red-950 dark:bg-red-950 p-3 rounded-lg border border-red-100 font-medium">
                  {questionsError}
                </p>
              )}
            </div>
