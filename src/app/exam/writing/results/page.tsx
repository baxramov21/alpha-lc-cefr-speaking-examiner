...
export default function WritingResultsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 flex flex-col pb-20">
      {/* Header */}
      ...
      <main className="max-w-4xl mx-auto w-full px-6 -mt-16 relative z-20 space-y-8">
        ...
        {/* Score Card */}
        <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-[var(--radius-lg)] shadow-xl p-8 border border-slate-100 dark:border-slate-800 dark:border-slate-800 flex flex-col md:flex-row items-center gap-8">
          <div className="flex-1 w-full">
            <div className="text-sm font-bold tracking-widest text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase mb-2">Overall Writing Score</div>
            <div className="flex items-end gap-4">
              <div className="text-6xl font-black text-slate-900 leading-none">{evaluation.total_score}</div>
              <div className="text-2xl font-bold text-slate-400 dark:text-slate-500 dark:text-slate-500 pb-1">/ 75</div>
            </div>
          ...
          <div className="w-px h-24 bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 hidden md:block" />
          ...
          <div className="flex-1 w-full grid grid-cols-3 gap-x-6 gap-y-4">
            ...
            <div className="col-span-1">
              <div className="text-xs font-semibold text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase mb-1">Grammar & Lexical</div>
              <div className="text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300">{evaluation.criteria_ratings.grammar_accuracy} / {evaluation.criteria_ratings.lexical_resource}</div>
            </div>
            <div className="col-span-2">
              <div className="text-xs font-semibold text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase mb-1">Task & Coherence</div>
              <div className="text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300">{evaluation.criteria_ratings.task_achievement} / {evaluation.criteria_ratings.coherence_cohesion}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-[var(--radius-lg)] p-6 shadow-sm border border-slate-100 dark:border-slate-800 dark:border-slate-800">
            <div className="flex items-center gap-3 mb-4">
              <div className="bg-emerald-100 p-2 rounded-lg text-emerald-600">
                <CheckCircle className="w-5 h-5" />
              </div>
              <h3 className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200 text-lg">Strengths</h3>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-[var(--radius-lg)] p-6 shadow-sm border border-slate-100 dark:border-slate-800 dark:border-slate-800">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200 text-lg">Areas to Improve</h3>
            </div>
            <ul className="space-y-3">
        ...
        <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-[var(--radius-lg)] shadow-sm border border-slate-100 dark:border-slate-800 dark:border-slate-800 overflow-hidden">
          <div className="flex border-b border-slate-100 dark:border-slate-800 dark:border-slate-800">
            ...
            {questions.length > 2 && (
              <button
                ...
                className={`flex-1 py-4 text-sm font-semibold transition-colors ${
                  activeTask === 1.2 ? 'bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 text-teal-600 border-b-2 border-teal-500' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:bg-slate-50'
                }`}
              >
                Task 1.2 Review
              </button>
          ...
          <div className="p-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">{activePrompt?.title}</h3>
              <div className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400 font-medium bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 px-3 py-1 rounded-md">
            ...
            <div className="mb-8">
              <div className="text-sm font-bold text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase mb-2">AI Feedback</div>
              <div className="bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 p-4 rounded-xl text-slate-700 dark:text-slate-300 dark:text-slate-300 text-sm leading-relaxed border border-slate-100 dark:border-slate-800 dark:border-slate-800">
                {activeEval.feedback}
              </div>
            </div>

            <div>
              <div className="text-sm font-bold text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase mb-2 flex items-center gap-2">
                <PenTool className="w-4 h-4" />
                Corrected Essay
              </div>
              <div 
                className="bg-white dark:bg-slate-900 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-700 p-6 rounded-xl text-slate-700 dark:text-slate-300 dark:text-slate-300 text-base leading-relaxed whitespace-pre-wrap font-serif"
                dangerouslySetInnerHTML={{ __html: activeEval.corrected_text_html }}
              />
              <div className="mt-4 flex gap-6 text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 inline-flex p-2 rounded-lg">
