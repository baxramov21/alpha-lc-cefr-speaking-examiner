  ...
  return (
    ...
    >
      ...
      <header className="bg-white dark:bg-slate-900 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm shrink-0">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 p-1 rounded-lg">
              ...
              {questions.length > 2 && (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${
                    currentTask === 1.2 ? 'bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-sm text-teal-600' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-700 '
                  }`}
                >
                  Task 1.2
                </button>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className={`flex items-center gap-2 font-mono text-lg font-bold ${timeLeft < 300 ? 'text-red-500 animate-pulse' : 'text-slate-700 dark:text-slate-300 dark:text-slate-300 '}`}>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 flex gap-6 overflow-hidden">
        ...
        {/* Left Panel: Prompt */}
        <div className="w-1/3 flex flex-col bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm overflow-hidden shrink-0">
          <div className="bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 dark:border-slate-800 px-6 py-4">
            <h2 className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200 ">{activeQuestion.title}</h2>
            <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-1 flex items-center gap-2">
              <span>Min {activeQuestion.minWords} words</span>
              <span>•</span>
              <span>~{activeQuestion.recommendedMinutes} minutes</span>
            </div>
          </div>
          <div className="p-6 overflow-y-auto grow text-slate-700 dark:text-slate-300 dark:text-slate-300 leading-relaxed whitespace-pre-wrap text-sm">
            {activeQuestion.imageUrl && (
              <div className="mb-6 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 ">
        ...
        <div className="w-2/3 flex flex-col bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm overflow-hidden">
          <textarea
            ...
            className="w-full grow p-6 resize-none focus:outline-none focus:ring-0 text-slate-700 dark:text-slate-300 dark:text-slate-300 text-base leading-relaxed"
            spellCheck={false}
          />
      ...
      {/* Loading Overlay */}
      {isSubmitting && (
        <div className="fixed inset-0 bg-white dark:bg-slate-900 dark:bg-slate-900  backdrop-blur-sm z-50 flex flex-col items-center justify-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200 mb-2">Analyzing your writing...</h2>
          <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 text-center max-w-sm">
