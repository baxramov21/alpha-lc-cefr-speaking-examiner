...
export default function WritingSessionPage() {
  ...
  }, [task1Text, task2Text, currentTask, isRestoring, sessionToken]);
    ...
    try {
      if (!task1Text.trim() && !task2Text.trim()) {
        const evaluation = {
          total_score: 0,
          cefr_level: 'Below B1',
          task_scores: { task_1_score: 0, task_2_score: 0 },
          criteria_ratings: {
      ...
      const res = await fetch('/api/evaluate-writing', {
        ...
        body: JSON.stringify({
          ...
          task2Prompt: questions[1]?.instructions || '',
        })
      });
  ...
  }, [sessionToken, task1Text, task2Text, router]);
  ...
  const activeQuestion = questions[currentTask - 1];
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 flex gap-6 overflow-hidden">
        ...
        <div className="w-2/3 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <textarea
            ...
            onChange={(e) => currentTask === 1 ? setTask1Text(e.target.value) : setTask2Text(e.target.value)}
            disabled={isSubmitting}
          ...
          <div className="bg-slate-50 border-t border-slate-100 px-6 py-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4">
              <span className={`text-sm font-semibold ${
                (currentTask === 1 ? t1Words : t2Words) < activeQuestion.minWords ? 'text-amber-500' : 'text-emerald-500'
              }`}>
                {currentTask === 1 ? t1Words : t2Words} words
              </span>
              {((currentTask === 1 ? t1Words : t2Words) < activeQuestion.minWords) && (
