        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
              ...
              {tasks.map((task, idx) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${
                    currentTaskIndex === idx ? 'bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-sm text-fuchsia-600' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-700 '
                  }`}
                >
                  Part {idx + 1}
                </button>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className={`flex items-center gap-2 font-mono text-xl font-bold px-4 py-1.5 rounded-lg transition-colors ${
              isWarning ? 'bg-red-50 dark:bg-red-950 dark:bg-red-950 text-red-600 animate-pulse' : 'bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 text-slate-700 dark:text-slate-300 dark:text-slate-300 '
            }`}>
              <Clock className={`w-5 h-5 ${isWarning ? 'text-red-500' : 'text-slate-400 dark:text-slate-500 dark:text-slate-500 '}`} />
              {timeString}
            </div>
            <Button 
              onClick={submitExam} 
          ...
          <div className="flex flex-col h-full bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 dark:border-slate-700 overflow-hidden">
            <div id="reading-questions-container" className="flex-1 overflow-y-auto p-6 lg:p-10">
              <div className="max-w-2xl mx-auto">
                <div className="mb-8 pb-6 border-b border-slate-200 dark:border-slate-700 dark:border-slate-700 ">
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200 dark:text-slate-200 mb-2">Questions</h2>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="space-y-3">
                          {getDisplayOptions(q).map((opt: string, i: number) => (
                            <label key={i} className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${
                              answers[q.id] === opt 
                                ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950 dark:bg-indigo-950 shadow-sm' 
                                : 'border-slate-200 dark:border-slate-700 dark:border-slate-700 hover:border-indigo-300 hover:bg-slate-50 '
                            }`}>
                              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mr-3 ${
                                answers[q.id] === opt ? 'border-indigo-500' : 'border-slate-300 dark:border-slate-600 dark:border-slate-600'
                              }`}>
                              ...
                              <input 
                                ...
                                className="hidden"
                              />
                              <span className={`font-medium ${answers[q.id] === opt ? 'text-indigo-900' : 'text-slate-700 dark:text-slate-300 dark:text-slate-300 '}`}>
                      ...
                      {q.type === 'fill_in' && (
                        <div className="mt-2">
                          <input 
                            type="text" 
                            className="w-full max-w-sm h-12 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 px-4 font-medium text-slate-800 dark:text-slate-200 dark:text-slate-200 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950 focus:bg-white focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-500/10 transition-all outline-none"
            ...
            <div className="mt-12 flex justify-between items-center pt-8 border-t border-slate-200 dark:border-slate-700 dark:border-slate-700 ">
              <Button 
                variant="outline" 
                onClick={() => setCurrentTaskIndex(i => Math.max(0, i - 1))}
                disabled={currentTaskIndex === 0}
                className="font-semibold text-slate-600 dark:text-slate-300 dark:text-slate-300 "
              >
                Previous Part
              </Button>
              <Button 
