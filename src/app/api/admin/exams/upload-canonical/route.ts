  ...
  try {
    ...
    const { data: examData, error: examError } = await supabase
      ...
      .insert({
        ...
        grammar_level: data.grammar_level || null,
        time_limit: data.time_limit || (data.exam_type === 'CEFR_READING' ? 3600 : 2400),
        prep_time: data.prep_time || (data.exam_type === 'CEFR_LISTENING' ? 10 : 0),
      })
      .select()
      .single();
    ...
    for (const part of data.parts) {
      ...
      if (part.image_url) {
        finalPassageHtml = `<img src="${part.image_url}" class="w-full max-w-2xl mx-auto rounded-2xl shadow-md my-6 border border-slate-200 dark:border-slate-700 dark:border-slate-700" alt="Part Diagram" />\n` + finalPassageHtml;
      }
