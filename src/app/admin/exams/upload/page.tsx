    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-8 p-6 bg-blue-50 border border-blue-200 rounded-2xl flex flex-col md:flex-row items-center gap-6 shadow-sm">
        ...
        <div className="w-full md:w-auto flex flex-col items-end gap-2">
           ...
           <label 
             htmlFor="image-host" 
             className={`px-4 py-2 rounded-xl font-bold transition-all shadow-sm cursor-pointer border ${isHostingImage ? 'bg-blue-100 text-blue-400 border-blue-200' : 'bg-white dark:bg-slate-900 text-blue-700 border-blue-300 hover:bg-blue-100'}`}
           >
      ...
      {hostedImageUrl && (
        <div className="mb-8 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700">
              <img src={hostedImageUrl} alt="Hosted" className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Image URL Generated</p>
              <p className="text-xs font-mono text-slate-500 dark:text-slate-400 dark:text-slate-500 max-w-md truncate">{hostedImageUrl}</p>
            </div>
          </div>
          <button 
            onClick={() => {
        ...
        <button
          ...
          className={`px-6 py-3 rounded-xl font-bold transition-all ${
            examMode === 'reading' 
              ...
              : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-950'
          }`}
        >
          Reading Exam
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        {examMode === 'listening' && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 overflow-hidden flex flex-col h-full">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4">Step 1: Upload Audio (MP3/WAV)</h3>
            <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-950 p-6 flex-1 flex flex-col items-center justify-center text-center transition-colors hover:bg-slate-100 dark:bg-slate-800 relative group">
              ...
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Select Audio Files</p>
        ...
        <div className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 overflow-hidden flex flex-col h-full ${examMode === 'reading' ? 'md:col-span-2' : ''}`}>
          ...
          <input 
            ...
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-50" 
          />
          <div className="w-14 h-14 bg-white dark:bg-slate-900 rounded-full shadow-sm flex items-center justify-center border border-slate-100 dark:border-slate-800 mb-4 group-hover:scale-110 transition-transform">
            <UploadCloud className="w-6 h-6 text-indigo-500" />
          </div>
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Click to upload or drag and drop</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Only .json format is supported</p>
          
          {file && (
            <div className="mt-4 flex items-center gap-2 text-xs font-medium text-indigo-700 bg-indigo-50 dark:bg-indigo-950 px-3 py-2 rounded-full border border-indigo-100 truncate max-w-full">
        ...
        {previewData && (
          <div className="mt-8 border-t border-slate-100 dark:border-slate-800 pt-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4 flex justify-between">
                  Passage Rendering
                  <span className="text-xs font-medium px-2 py-0.5 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded">{previewData.exam_type}</span>
                </h4>
                <h1 className="text-xl font-bold mb-4">{previewData.title}</h1>
                <div 
                  className="prose prose-sm max-w-none bg-white dark:bg-slate-900 p-4 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800 h-96 overflow-y-auto"
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(previewData.parts?.[0]?.passage_html || '') }}
                />
              </div>

              <div className="bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">Extracted Questions ({previewData.parts?.reduce((acc: number, p: any) => acc + (p.questions?.length || 0), 0) || 0})</h4>
                <div className="space-y-3 h-96 overflow-y-auto pr-2">
                  {previewData.parts?.map((p: any) => p.questions || []).flat().map((q: any, i: number) => (
                    <div key={i} className="bg-white dark:bg-slate-900 p-4 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold text-slate-400 dark:text-slate-500">Q{q.question_number}</span>
                        <span className="text-[10px] font-semibold tracking-wider uppercase bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500 px-2 py-1 rounded">{q.type}</span>
                      </div>
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mb-3">{q.question_text}</p>
                      {q.options && q.options.length > 0 && (
                        <div className="space-y-1.5 mb-3">
                          {q.options.map((opt: string, j: number) => (
                            <div key={j} className="text-xs flex gap-2">
                              <span className="text-slate-400 dark:text-slate-500 font-mono">{String.fromCharCode(65 + j)}.</span>
                              <span className="text-slate-600 dark:text-slate-300">{opt}</span>
                            </div>
                          ))}
                        </div>
                      )}
                      <div className="text-xs font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-950 px-2 py-1 rounded inline-block">
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-sm w-full mx-4 animate-in zoom-in-95 duration-200">
            ...
            {examMode === 'listening' && audioFiles.length > 0 ? (
              <div className="w-full text-center">
                <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm mb-6">Processing audio files and compiling JSON data...</p>
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-4 mb-2 overflow-hidden shadow-inner">
                  <div 
              ...
              <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm text-center">Saving canonical exam data to the database...</p>
            )}
          </div>
        </div>
      )}
