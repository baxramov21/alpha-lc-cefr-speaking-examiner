  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl">
      ...
      <Link
        ...
        className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-slate-700 dark:text-slate-300 transition-colors"
      >
        <ArrowLeft className="w-4 h-4" /> All Submissions
      </Link>

      {/* Student metadata card */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">{submission.status.toUpperCase()}</div>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 space-y-3">
        <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm uppercase tracking-wide">Examiner / Admin Notes</h3>
        <textarea
          value={adminNotes}
          onChange={(e) => setAdminNotes(e.target.value)}
          placeholder="Add private notes about this student's performance..."
          className="w-full min-h-[100px] rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-sm focus:ring-2 focus:ring-teal-500 outline-none"
        />
      ...
      {ev.examType === 'writing' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 text-center">
            <h3 className="text-slate-400 dark:text-slate-500 text-sm font-semibold mb-2 uppercase tracking-wide">Task 1 (Letter)</h3>
            <div className="text-3xl font-bold text-slate-800 dark:text-slate-200">{ev.task_scores?.task_1_score || 0} <span className="text-base text-slate-400 dark:text-slate-500 font-medium">/ 24</span></div>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 text-center">
            <h3 className="text-slate-400 dark:text-slate-500 text-sm font-semibold mb-2 uppercase tracking-wide">Task 2 (Essay)</h3>
            <div className="text-3xl font-bold text-slate-800 dark:text-slate-200">{ev.task_scores?.task_2_score || 0} <span className="text-base text-slate-400 dark:text-slate-500 font-medium">/ 51</span></div>
          </div>
        </div>
      ...
      {ev.examType === 'writing' ? (
        <div className="space-y-6">
          <h3 className="font-bold text-slate-800 dark:text-slate-200 text-xl">Essay Analysis</h3>
          {[
            { id: 'Task 1', title: 'Formal Letter', eval: ev.task_1_eval },
            { id: 'Task 2', title: 'Essay', eval: ev.task_2_eval }
          ].map((task) => (
            <div key={task.id} className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-4">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 text-sm leading-relaxed mt-2 text-slate-700 dark:text-slate-300">
        ...
        <div className="space-y-6">
          ...
          {ev.question_results?.map((qr: any, idx: number) => (
            <div key={idx} className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <Badge variant="outline" className="text-slate-500 dark:text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700">
                    Q{idx + 1}
                  </Badge>
                  {qr.is_correct ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wide">Student Answer</span>
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{qr.user_answer || '[No Answer]'}</p>
                </div>
                <div className="p-4 bg-emerald-50 dark:bg-emerald-950 rounded-xl border border-emerald-100/50 flex flex-col gap-2">
