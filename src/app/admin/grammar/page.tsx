    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Manage grammar quizzes and bundles for students.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <Button onClick={fetchExams} variant="outline" className="gap-2 text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900">
      ...
      <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit mb-8">
        <button
          onClick={() => setActiveTab('grammar')}
          className={`px-5 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'grammar' ? 'bg-white dark:bg-slate-900 shadow text-slate-900' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
          <span className="flex items-center gap-2"><Layers className="w-4 h-4" /> Pure Grammar</span>
        </button>
        <button
          onClick={() => setActiveTab('reading')}
          className={`px-5 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'reading' ? 'bg-white dark:bg-slate-900 shadow text-slate-900' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
          <span className="flex items-center gap-2"><Layers className="w-4 h-4" /> Reading</span>
        </button>
        <button
          onClick={() => setActiveTab('listening')}
          className={`px-5 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'listening' ? 'bg-white dark:bg-slate-900 shadow text-slate-900' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
          <span className="flex items-center gap-2"><Layers className="w-4 h-4" /> Listening</span>
        </button>
        <button
          onClick={() => setActiveTab('triples')}
          className={`px-5 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'triples' ? 'bg-white dark:bg-slate-900 shadow text-slate-900' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
      ...
      {activeTab === 'triples' && (
        <div className="bg-white dark:bg-slate-900 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm p-6 mb-8">
          ...
          {isCreatingTriple && (
            <form onSubmit={handleCreateTriple} className="bg-slate-50 dark:bg-slate-950 p-6 rounded-xl border border-slate-100 dark:border-slate-800 mb-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Pair Name</label>
                  <input required value={tripleForm.name} onChange={e => setTripleForm({...tripleForm, name: e.target.value})} className="w-full border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2" placeholder="e.g. End of Month Test" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Level</label>
                  <select value={tripleForm.level} onChange={e => setTripleForm({...tripleForm, level: e.target.value})} className="w-full border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Reading Test</label>
                  <select required value={tripleForm.reading_exam_id} onChange={e => setTripleForm({...tripleForm, reading_exam_id: e.target.value})} className="w-full border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Listening Test</label>
                  <select required value={tripleForm.listening_exam_id} onChange={e => setTripleForm({...tripleForm, listening_exam_id: e.target.value})} className="w-full border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Grammar Test</label>
                  <select required value={tripleForm.grammar_exam_id} onChange={e => setTripleForm({...tripleForm, grammar_exam_id: e.target.value})} className="w-full border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm">
          ...
          {triples.length === 0 && !isLoading ? (
            <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-center py-8">No pairs created yet.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {triples.map(trip => (
                <div key={trip.id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-5 hover:border-indigo-200 transition-colors bg-slate-50 dark:bg-slate-950/50">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">{trip.level}</span>
                    </div>
                  ...
                  <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
                    <p><strong className="text-slate-800 dark:text-slate-200 font-medium">R:</strong> {trip.reading_exam?.title || 'None'}</p>
                    <p><strong className="text-slate-800 dark:text-slate-200 font-medium">L:</strong> {trip.listening_exam?.title || 'None'}</p>
                    <p><strong className="text-slate-800 dark:text-slate-200 font-medium">G:</strong> {trip.grammar_exam?.title || 'None'}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab !== 'triples' && (
        <div className="bg-white dark:bg-slate-900 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
          {isLoading ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
                    <th className="py-4 pl-6 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Status</th>
                ...
                <tbody>
                  {displayedExams.map((exam) => (
                    <tr key={exam.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 dark:bg-slate-950 transition-colors">
                      <td className="py-4 pl-6">
                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${exam.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500'}`}>
                        ...
                        {editingTitle?.id === exam.id ? (
                          <div className="flex gap-2 items-center">
                            <input 
                              ...
                              className="border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm bg-white dark:bg-slate-900 w-48"
                            />
                        ...
                        {editingExam?.id === exam.id ? (
                          <div className="flex gap-2 items-center">
                            <select 
                              ...
                              className="border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm bg-white dark:bg-slate-900"
                            >
                              <option value="Elementary">Elementary</option>
                              <option value="Pre-Intermediate">Pre-Intermediate</option>
                              <option value="Intermediate">Intermediate</option>
                          ...
                          <div className="flex items-center gap-2 group">
                            <span className="capitalize text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
                      ...
                      <td className="py-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button 
                            ...
                            variant="outline" 
                            size="sm"
                            className={exam.is_active ? 'text-amber-600 hover:text-amber-700 hover:bg-amber-50 dark:bg-amber-950 border-amber-200' : 'text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:bg-emerald-950 border-emerald-200'}
                          >
                          ...
                          <Button 
                            ...
                            variant="outline" 
                            size="sm"
                            className="text-red-500 hover:text-red-700 hover:bg-red-50 dark:bg-red-950 border-red-200 h-9 w-9 p-0"
                          >
