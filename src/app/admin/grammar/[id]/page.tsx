...
export default function EditGrammarExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div className="p-12 text-center">
      ...
      <p className="text-slate-500 font-medium">Loading grammar exam editor...</p>
    </div>
  );

  if (error && !exam.title) return (
    <div className="p-12 text-center text-red-500 font-bold bg-red-50 rounded-2xl border border-red-100 m-8">
      {error}
      <br />
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {success && (
              <span className="flex items-center text-emerald-600 text-sm font-bold bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-200 rounded-xl px-4 py-3 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
              />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Time Limit (seconds)</label>
              <input
                type="number"
                ...
                className="w-full border border-slate-200 rounded-xl px-4 py-3 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
              />
              <p className="text-xs text-slate-400 mt-1">E.g., 2400 for 40 minutes.</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {exam.questions.map((q: any, qIdx: number) => (
            <div key={qIdx} className="bg-white p-6 lg:p-8 rounded-3xl shadow-sm border border-slate-200 relative group">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Number</label>
                  <input
                    type="number"
                    value={q.question_number}
                    onChange={(e) => updateQuestion(qIdx, 'question_number', parseInt(e.target.value) || 0)}
                    className="w-full md:w-32 border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 text-sm"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 text-sm"
                  >
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 bg-slate-50 focus:bg-white min-h-[80px]"
                />
              </div>
              ...
              <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-4">
                ...
                {q.type === 'MULTIPLE_CHOICE' ? (
                  <select
                    ...
                    className="w-full border border-emerald-200 rounded-lg px-3 py-2 bg-white text-sm"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 bg-slate-50 focus:bg-white min-h-[60px] text-sm"
          ...
          <Button onClick={addQuestion} variant="outline" className="w-full bg-white border-dashed border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold h-12 rounded-2xl">
            <Plus className="w-4 h-4 mr-2" /> Add Question
          </Button>
